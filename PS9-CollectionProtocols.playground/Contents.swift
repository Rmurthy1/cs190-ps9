...
struct LocationTrack {
    ...
    var totalLength: CLLocationDistance {
        ...
        for segment in segmentSequence {
            result = result + segment.start.distanceFromLocation(segment.end)
        }
        return result
    }
...
class SegmentGenerator: GeneratorType {
    ...
    func next() -> Segment? {
        if (index < self.locationTrack.locations.count - 1)
        {
            index = index + 1
            return Segment(locationTrack.locations[index - 1], locationTrack.locations[index])
        }
